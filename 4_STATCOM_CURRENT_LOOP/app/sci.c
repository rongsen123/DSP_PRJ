...
#define MASTER_REQUEST_VDC_THRESHOLD 5
/*
 * 串口接收数据帧结构体
 */
typedef struct
...
static Uint16 master_threshold_value = 3410U;
...
static void master_send_vdc_threshold(Uint16 raw_limit);
...
static Uint16 sci_take_frame(SCI_RX_PORT *port, Uint8 *frame)
{
    ...
    /* Single producer (RX ISR) / single consumer (foreground): the ISR only
     * advances head and never touches tail.  Snapshot the 16-bit atomic head
     * and copy without globally masking the 20 kHz ADC interrupt.  Bytes that
     * arrive after this snapshot remain queued for the next frame. */
    head_snapshot = port->head;
    while((port->tail != head_snapshot) && (count < MODBUS_FRAME_MAX))
    {
...
/* Apply the host-selected physical Vdc threshold as a local CPLD raw limit. */
static void master_send_vdc_threshold(Uint16 raw_limit)
{
    Uint8 frame[8];
    frame[0] = MODBUS_CPLD_ADDRESS;
    frame[1] = 0x06;
    put_be16(&frame[2], 0x1000);
    put_be16(&frame[4], raw_limit);
    append_crc(frame, 6);
    sci_write_a(frame, 8);
    master_threshold_value = raw_limit;
    master_waiting = 1;
    master_request = MASTER_REQUEST_VDC_THRESHOLD;
    master_sent_ms = system_millis;
}
/*
 * 根据状态机决定主站写还是读
...
static void master_resend(void)
{
    ...
    else if(master_request == MASTER_REQUEST_VDC_THRESHOLD)
    {
        master_send_vdc_threshold(master_threshold_value);
    }
    else
    {
        master_send_read();
...
static void master_handle_frame(const Uint8 *frame, Uint16 length)
{
    ...
    if((frame[1] & 0x80) != 0)
    {
        ...
        if(master_request == MASTER_REQUEST_VDC_THRESHOLD)
        {
            if(cpld_link_command.vdc_over_limit_raw == master_threshold_value)
            {
                cpld_link_command.vdc_threshold_request = 0U;
            }
        }
        master_waiting = 0;
            ...
            (get_be16(&frame[4]) == MODBUS_RESET_CPLD_KEY))
    {
        ...
        master_next_poll_ms = system_millis + CPLD_POLL_PERIOD_MS;
            ...
            (frame[1] == 0x06) && (length == 8) &&
            (get_be16(&frame[2]) == 0x1000) &&
            (get_be16(&frame[4]) == master_threshold_value))
    {
        if(cpld_link_command.vdc_over_limit_raw == master_threshold_value)
        {
            cpld_link_command.vdc_threshold_request = 0U;
...
static void master_service(void)
{
    ...
    if(master_waiting != 0)
    {
        ...
        if((system_millis - master_sent_ms) >= CPLD_RESPONSE_TIMEOUT_MS)
        {
            ...
            else
            {
                ...
                if(master_request == MASTER_REQUEST_VDC_THRESHOLD)
                {
                    if(cpld_link_command.vdc_over_limit_raw ==
                       master_threshold_value)
                    {
                        cpld_link_command.vdc_threshold_request = 0U;
                    }
                }
    ...
    if((system_millis - master_next_poll_ms) < 0x80000000)
    {
        ...
        else if(cpld_link_command.vdc_threshold_request != 0U)
        {
            master_send_vdc_threshold(cpld_link_command.vdc_over_limit_raw);
        }
        else
        {
            master_send_read();
...
static Uint16 dsp_input_register(Uint16 address, Uint16 *value)
{
    ...
    case 0x0007: *value = adc_zero_calibration.iac_offset_count; break;
    ...
    case 0x000C: *value = dsp_protection_latched_flags; break;
...
static Uint16 dsp_holding_register(Uint16 address, Uint16 *value)
{
    ...
    if(protection_read_threshold(address, value) != 0U)
    {
        return 1;
    }
    return 0;
...
static Uint16 dsp_write_holding(Uint16 address, Uint16 value)
{
    ...
    if((address >= PROTECTION_REG_VDC_OV_RAW) &&
       (address <= PROTECTION_REG_IAC_OV_DELTA_RAW))
    {
        Uint16 protection_result;
        if((cpld_link_command.run_enable != 0U) ||
           (cpld_link_command.clear_fault_request != 0U) ||
           (cpld_link_command.reset_request != 0U))
        {
            return 2U;
        }
        protection_result = protection_write_threshold(address, value);
        if((protection_result == 1U) &&
           (address == PROTECTION_REG_VDC_OV_RAW))
        {
            cpld_link_command.vdc_over_limit_raw = value;
            cpld_link_command.vdc_threshold_request = 1U;
        }
